...
Механизм за первым выводом удалось померить, а не предположить (§9.7): **ACCEL с MaxMC копит
...
3. **Одношаговая мутация распределение не двигает.** Медиана детей — те же 11 шагов. Значит вся
   ...
   отдельно, и у MaxMC её не оказалось (§9.7).
...
### 9.7. Лестница ACCEL: оператор её может, MaxMC её не просит
...
Меряется это двумя измерениями, и оба нужны, потому что порознь каждое отвечает не на тот вопрос.

**Что оператор может сам по себе** (`level_diagnostics --ladder`, агента нет): 1024 уровня, по 5
правок за раунд. Рука `random` — ребёнок безусловно заменяет родителя. Рука `hardest_of_8` — 8
детей на родителя, выживает тот, у кого длиннее оптимальный путь; сломанные уровни получают −1 и
выиграть не могут. Вторая рука — потолок, недостижимый для настоящего teacher'а: она отбирает по
истинной длине пути, которую он никогда не видит.
...
**Вывод получается из совмещения таблиц.** Стены растут с глубиной линии примерно линейно, так что
по ним оси можно грубо сопоставить. При одинаковом числе стен (~40–50):
...
§3 показал, что обучающее и целевое распределения почти не пересекаются по сложности: медиана 11
...
2. **Многошаговая мутация.** Раз одношаговый ребёнок неотличим от родителя, а лестница у MaxMC не
   строится вовсе (§9.7) — проверить `k`-шаговую мутацию с растущим `k`. Верхняя рука ладдера
...
5. **Сиды для cheap-SFL и абляций** (§11.3–11.4) — прежде любых новых идей.
