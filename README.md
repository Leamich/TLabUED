...
стены, но не сложность** — медиана оптимального маршрута в его буфере за 30000 апдейтов растёт с
11 до 13 шагов, ровно как у слепой мутации без всякого отбора. Learnability поднимает её до 19,
...
| **`sfl_oracle`** — 64 лучших из 8192 по предсказанию | 8 192 | 64 (960) | 14 399 | **0.829** |
| `sfl_accel` — 224 случайных, полное измерение | 224 | 224 (3 360) | 13 199 | 0.829 |

Первые две строки отличаются **только** тем, как выбраны измеряемые 64: бюджет у них совпадает до
апдейта — и измерительный, и градиентный. Третья показывает, чего стоит купить сопоставимый отбор
одним измерением: 2 400 градиентных апдейтов, отданных фазе.

Укоротить фазу без oracle стоит 0.10 solve rate; oracle это возвращает. То есть ранжирование
действительно подменяет измерение — но потолок ставит не оно. Оговорка, без которой первую строку
читать нельзя: `sfl_accel_cheap` — один сид, а `sfl_oracle` по трём сидам разбросан в 0.715–0.915.
Разница 0.731 против 0.829 лежит внутри этого разброса, так что это подтверждённое направление, а
не измеренный эффект (§8.4).
...
Прежде чем что-то менять, стоит выяснить, чем именно ACCEL занят в этом домене. Три вопроса, и
все три меряются без единого прогона обучения: что предлагает генератор, что умеет оператор
мутации и есть ли основания подозревать score-функцию. Ответы объясняют большую часть результатов
ниже — в частности, почему один из ожидаемых механизмов победы оказался невозможен.
...
### 3.2. На что способен оператор мутации

Из §3.1 следует, что вся надежда ACCEL — на лестницу, и первый вопрос к ней: **умеет ли оператор
строить трудные уровни в принципе**. Это свойство оператора, а не курикулума, поэтому меряется оно
без агента.

Измерение (`level_diagnostics --ladder`): 1024 уровня, по 5 правок за раунд, две руки. `random` —
ребёнок безусловно заменяет родителя, то есть мутация вообще без отбора. `hardest_of_8` — 8 детей
на родителя, выживает тот, у кого длиннее оптимальный путь; сломанные уровни получают −1 и
выиграть не могут.

| раундов | `random`: медиана / решаемых / стены | `hardest_of_8`: медиана / решаемых / стены |
|---|---|---|
| 0 | 11 / 99.3% / 23 | 11 / 99.3% / 23 |
| 8 | 12 / 98.1% / 32 | 22 / 100% / 34 |
| 18 | 13 / 94.4% / 42 | 29 / 100% / 46 |
| 50 | 14 / 61.5% / 61 | 40 / 99.9% / 64 |

Слепая мутация маршрут почти не удлиняет (11 → 14 за 250 правок), зато исправно замуровывает цель:
решаемость падает до 61%. Отбор по сложности — 11 → 40 при сохранной решаемости. **Оператор умеет
строить трудные лабиринты, если его об этом просят.**

Обе руки — границы, между которыми обязан лечь настоящий курикулум. Верхняя ему недостижима: она
отбирает по истинной длине пути, которую teacher никогда не видит. Где именно между ними легли
ACCEL и SFL — §7.7.

### 3.3. Почему score-функция под подозрением
...
### 4.2. SFL-ORACLE: предсказывать `p` вместо того, чтобы его покупать
...
Каждые 250 апдейтов student прогоняется по восьми held-out уровням, по 30 попыток на уровень;
solve rate — доля эпизодов, дошедших до цели. Основное число в отчёте — **среднее по последним 20
эвалам**, а не по последнему: восемь уровней × 30 попыток — маленькая выборка, и соседние эвалы
...
### 7.7. Лестница: оператор её может, MaxMC её не просит
...
мутируется снова, и сложность накапливается лестницей. Из обучающих метрик это не видно: там
только число стен. §3.2 показал, что оператор трудные лабиринты строить умеет; вопрос в том,
просит ли его об этом score-функция.

**Что курикулум взял на самом деле** (`buffer_diagnostics`, агент в петле): буфер уровней лежит в
каждом чекпоинте целиком, так что распределение, отобранное student'ом, читается задним числом —
без перезапуска и без GPU. Медиана оптимальных шагов по буферу, от первого чекпоинта к последнему:
...
**Вывод получается из совмещения этой таблицы с §3.2.** Стены растут с глубиной линии примерно
линейно, так что по ним оси двух измерений можно грубо сопоставить. При одинаковом числе стен
(~40–50):

| | медиана оптимальных шагов |
|---|---|
| слепая мутация, отбора нет | 13–14 |
| `accel` (MaxMC) | 13–16 |
| `sfl_accel` (learnability) | 19 |
| `sfl_oracle` | 21–26 |
| потолок: отбор прямо по длине пути | 29–33 |

**ACCEL с MaxMC поднимается по лестнице не быстрее случайной мутации.** Стены он копит, маршруты —
нет: лестница есть в описании метода, но в этом домене её нет в данных, и рост
`level/mean_num_blocks` с 25 до 47 означал ровно то, что написано, — больше стен, а не более
трудные уровни. Learnability лестницу запускает, oracle доводит её до ~70–80% потолка, достижимого
отбором по истинной длине пути.
...
против маршрутов на порядок длиннее. Learnability этот разрыв частично **закрывает** — медиана
буфера к концу обучения втрое выше стартовой, — а MaxMC оставляет student'а на тех же уровнях, на
которых он начинал.
...
score-функции. Сопоставление по числу стен — прокси для глубины линии, а не её измерение;
настоящую глубину дал бы счётчик поколений в `teacher_state` и новые прогоны. `sfl_accel` — один
сид.
...
2. **Многошаговая мутация.** Раз одношаговый ребёнок неотличим от родителя, а лестница у MaxMC не
   строится вовсе (§7.7) — проверить `k`-шаговую мутацию с растущим `k`. Верхняя рука ладдера
   (`hardest_of_8`: медиана 40 шагов за 50 раундов) показывает, что запас у оператора есть.
