...
# `after_rollout` gets a key folded off the branch's rather than split from it,
# so that installing a hook leaves this teacher's own PRNG stream untouched and
# its runs comparable with the ones made before the hook existed.
HOOK_SALT = 0x0A17
...
class SFLACCELTeacher(ACCELTeacher):
    ...
    # --- extension points -----------------------------------------------------
    # Two hooks, both no-ops here, both used by `teachers/sfl_oracle.py`. They
    # exist so that a teacher which *learns* from the curriculum does not have to
    # fork three branch bodies to see the data flowing through them.

    def after_rollout(
        self, rng: chex.PRNGKey, ts: Dict[str, Any], levels, attempts, successes
    ) -> Dict[str, Any]:
        """Called once per update with what that update's rollout measured.

        `successes` out of `attempts` episodes reached the goal on each of
        `levels` - one Bernoulli sample per level on the replay branch, `k` on the
        scoring branches. Returns the teacher state to carry forward.
        """
        return ts

    def attempt_counts(self, success_rate: chex.Array):
        """`(attempts, successes)` for `after_rollout`, from a k-attempt rate."""
        n = jnp.full(success_rate.shape, float(self.attempts), dtype=jnp.float32)
        return n, success_rate * n

    def propose_children(self, rng: chex.PRNGKey, train_state: TrainState, parents):
        """The mutated levels a mutation update should evaluate. One child each."""
        return jax.vmap(self.ctx.mutate_level, (0, 0, None))(
            jax.random.split(rng, self.levels_per_update), parents, self.config["num_edits"]
        )
    ...
    def on_replay_levels(self, rng: chex.PRNGKey, train_state: TrainState):
        ...
        solved = first_episode_success(dones, rewards)
        success_rate = (1.0 - self.p_decay) * extra["p"] + self.p_decay * solved
        scores = self.score_from_success(success_rate)
        ...
        # One episode per level here, so the label is the raw Bernoulli sample -
        # not the decayed estimate the score is computed from.
        ts = self.after_rollout(
            jax.random.fold_in(rng, HOOK_SALT), ts, levels, jnp.ones_like(solved), solved
        )
        train_state = train_state.replace(
            teacher_state={
                **ts,
    ...
    def on_mutate_levels(self, rng: chex.PRNGKey, train_state: TrainState):
        ...
        children = self.propose_children(rng_mutate, train_state, parents)
        carry, traj, advantages, targets, batch, success_rate, max_returns = self.evaluate_levels(
        ...
        ts = self.after_rollout(
            jax.random.fold_in(rng, HOOK_SALT), ts, children, *self.attempt_counts(success_rate)
        )
        train_state = train_state.replace(
            teacher_state={
                **ts,
